...
if [ ! -d "/opt/lucee" ]; then
	...
	if [ -f "/vagrant/artifacts/lucee-4.5.1.022-pl1-linux-x64-installer.run" ]; then
		echo "... Copying Lucee installer ..."
		sudo cp /vagrant/artifacts/lucee-4.5.1.022-pl1-linux-x64-installer.run /root
	else
		echo "... Downloading the Lucee installer, standby ..."
		wget -O /root/lucee-4.5.1.022-pl1-linux-x64-installer.run http://railo.viviotech.net/downloader.cfm/id/133/file/lucee-4.5.1.022-pl1-linux-x64-installer.run &> /dev/null
		sudo cp /root/lucee-4.5.1.022-pl1-linux-x64-installer.run /vagrant/artifacts
	...
	chmod +x /root/lucee-4.5.1.022-pl1-linux-x64-installer.run
	cp /vagrant/configs/lucee-options.txt /root
	/root/lucee-4.5.1.022-pl1-linux-x64-installer.run --mode unattended --optionfile /root/lucee-options.txt
fi
